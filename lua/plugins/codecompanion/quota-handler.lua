...
function M.init()
  ...
  -- Create autocommand to monitor for quota issues
  vim.api.nvim_create_autocmd("User", {
    pattern = "CodeCompanionRequestFinished",
    group = vim.api.nvim_create_augroup("CodeCompanionQuotaHandler", { clear = true }),
    callback = function(ev)
      if ev.data and ev.data.status == "error" then
        local error_msg = ev.data.error or ""
        if error_msg:match("premium") or error_msg:match("quota") or error_msg:match("limit") then
          vim.notify("Detected quota error in request. Future requests will use GPT-4.1.", vim.log.levels.INFO)
...
function M.handle_premium_quota_error(self, data)
  ...
  if data and data.status >= 400 then
    ...
    local is_quota_error = body:match("premium") or body:match("quota") or 
                          body:match("limit") or body:match("exceeded") or
                          body:match("rate limit") or body:match("usage limit") or
                          body:match("Rate limit") or body:match("Usage limit")
    ...
    schema = {
      model = {
        default = function()
          ...
          if vim.g.codecompanion_fallback_model then
            return vim.g.codecompanion_fallback_model
          end

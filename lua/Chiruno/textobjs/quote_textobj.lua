function Chiruno.textobjs.is_inside(pos, selections)
  local buffer = require "nvim-surround.buffer"
  return buffer.comes_before(selections.first_pos, pos) and buffer.comes_before(pos, selections.last_pos)
end

function Chiruno.textobjs.filter_selections_list(selections_list)
  local buffer = require "nvim-surround.buffer"
  ...
  for _, cur_selections in ipairs(selections_list) do
    ...
    then
      ...
      if Chiruno.textobjs.is_inside(curpos, best_selections) then
        -- Handle case where the cursor is inside the nearest selections
        if
          Chiruno.textobjs.is_inside(curpos, cur_selections)
...
---@param mode "i"|"a"
function Chiruno.textobjs.quote_textobj(mode)
  local buffer = require "nvim-surround.buffer"
  ...
  local nearest_selections = Chiruno.textobjs.filter_selections_list(selections_list)
...
return Chiruno.textobjs.quote_textobj
